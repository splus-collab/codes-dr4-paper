...
field = Table.read('data/detections_in_bands_field.csv')

# make 4x3 grid using gridspec

fig = plt.figure(constrained_layout=True, figsize=(16, 8), dpi=100)
...
for i,band in enumerate(['u','J0378','J0395','J0410','J0430','g','J0515','r','J0660','i','J0861','z']):
    ...
    ax_list[i].plot(range(14,24), band2r_psf, label="psf", color=np.array(colors[band])/255, ls='--', lw=3, marker='s', markersize=8)
    ax_list[i].plot(range(14,24), band2r_single, label="single", color=np.array(colors[band])/255, ls=':', lw=3, marker='^', markersize=8)
    ax_list[i].plot(range(14,24), band2r_dual, label='dual', color=np.array(colors[band])/255, lw=3, ls='-', marker='o', markersize=8)
